...
@section('content')
    <div class="row mx-1">
        ...
        <div class="card">
            <h3 class="card-head my-2 mx-2 mt-3">Category</h3>
            <div class="card-body mx-2">
                <div class="d-flex">
                    ...
                    <div class="p-2">
                        <a href="{{ route('blog.category.index') }}" class="btn btn-info">Refresh</a>
                    </div>
                    <div class="ms-auto p-2">{{ $category->links() }}</div>
                </div>
                        ...
                        <tbody class="table-border-bottom-0">
                            @if ($category->count())
                                @foreach ($category as $item)
                                    <tr>
                                        <td align="center">{{ $category->firstItem() - 1 + $loop->iteration }}</td>
                                        <td>{{ $item->category }}</td>
                                        ...
                                        <div class="modal-dialog modal-sm" role="document">
                                            <div class="modal-content">
                                                <div class="modal-header">
                                                    <h5 class="modal-title" id="exampleModalLabel2">Edit Category
                                                        {{ $item->name }}</h5>
                                                </div>
                                                <form action="{{ route('blog.category.update', $item->id) }}"
                                                    method="post">
                                                    @method('put')
                                                    @csrf
                    ...
                    <div class="modal-body">
                        <div class="row">
                            <div class="col mb-3">
                                <label for="category" class="form-label">Category</label>
                                <input type="text" name="category" id="category"
                                    class="form-control @error('category') is-invalid @enderror" placeholder="Event"
                                    value="{{ old('category') }}" />
                            </div>
                        </div>
                        <div class="row">

...
@section('content')
    <div class="row mx-1">
        ...
        <div class="card">
            <h3 class="card-head my-2 mx-2 mt-3">Edit Blog {{ $blog->title }}</h3>
                ...
                <form action="{{ route('blog.update', $blog->id) }}" method="post" enctype="multipart/form-data">
                    ...
                    <div class="row mb-3">
                        ...
                        <div class="col-sm-10">
                            <input type="text" class="form-control @error('title') is-invalid @enderror" name="title"
                                id="title" placeholder="Blog's PT Aliansi Prima Energi" value="{{ old('title', $blog->title) }}" />
                            @error('title')
                    ...
                    <div class="row mb-3">
                        <label class="col-sm-2 col-form-label" for="article">Article <span
                                class="text-danger fw-bolder">*</span></label>
                        <div class="col-sm-10">
                            <textarea name="article" class="ckeditor" id="article">{{ old('article', $blog->article) }}</textarea>
                        </div>
                    </div>
                    <div class="row mb-3">
                        <label class="col-sm-2 col-form-label" for="subarticle">Sub Article </label>
                        <div class="col-sm-10">
                            <textarea name="subarticle" id="subarticle" class="form-control @error('subarticle') is-invalid @enderror"
                                cols="30" rows="5">{{ old('subarticle', $blog->subarticle) }}</textarea>
                            @error('subarticle')
                                <div class="form-text text-danger">
                                    {{ $message }}
                                </div>
                    ...
                    <div class="row mb-3">
                        ...
                        <div class="col-sm-10">
                            <textarea name="paragraph" id="paragraph" class="form-control @error('paragraph') is-invalid @enderror" cols="30"
                                rows="5">{{ old('paragraph', $blog->paragraph) }}</textarea>
                            @error('paragraph')
                                <div class="form-text text-danger">
                                    {{ $message }}
                                </div>
                    ...
                    <div class="row mb-3">
                        ...
                        <div class="col-sm-10">
                            <img src="{{ $blog->image ? asset('storage/' . $blog->image) : asset('assets/img/Kuroyasha.png') }}"
                                    ...
                                    @foreach ($category as $item)
                                        <div class="form-check form-check-inline mb-3">
                                            <input class="form-check-input" type="checkbox" name="category[]"
                                                ...
                                                {{ in_array($item->id, BlogHelper::category($blog->id)) ? 'checked' : '' }} />
                                    ...
                                    @foreach ($tag as $item)
                                        <div class="form-check form-check-inline mb-3">
                                            <input class="form-check-input" type="checkbox" name="tag[]"
                                                id="{{ $item->id }}" value="{{ $item->id }}"
                                                {{ in_array($item->id, BlogHelper::tag($blog->id)) ? 'checked' : '' }} />
                                            <label class="form-check-label"
